...
hist(main_df$inverted_faces_reported) 
hist(main_df$scrambled_faces_reported)
...
```{r}
#during revision I found that a few subjects had the wrong phenotype designation.
# These are currently being fixed in the database, so let's just fix
# them by hand here
#just for posterity here are the reasons these folks should not be HC:
#20366 and 50458 have current scid diagnoses 
#20622 is actively on antidepressants and has lifetime MDD diagnosis
#10244 is currently taking stimulants and has a scid diagnosis of adult adhd.
# 50090 had subthreshold MDD (which I think is okay), but is currently taking
#       anxiolytics (Buspirone)
main_df$phenotype[main_df$src_subject_id %in% 
                    c(20366,50458,20622,10244,50090)] <- 'hsc'



main_df$help_index = main_df$helpful_index #something weird about this variable label
main_df$phenotype_final<-
  ifelse(main_df$phenotype=="hsc",
