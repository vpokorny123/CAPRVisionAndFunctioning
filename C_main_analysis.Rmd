...
summary(lm(context_sensitivity ~ wrat_standardscore + bacs_total + hvlt_total_score, main_df))
...
I guess by this I just meant throw them all in the same model and see what predicts best?
```{r}
summary(lm(upright_faces_reported ~ gfs_current+gfr_current + sps_total +demo_age_in_years +
             wrat_standardscore + bacs_total + hvlt_total_score, main_df))
summary(lm(inverted_faces_reported ~ gfs_current+gfr_current + sps_total +demo_age_in_years +
             wrat_standardscore + bacs_total + hvlt_total_score, main_df))
summary(lm(context_sensitivity ~ gfs_current + gfr_current + sps_total +demo_age_in_years +
             wrat_standardscore + bacs_total + hvlt_total_score, main_df))
```
3. Now let's look at group differences?
```{r}
library(afex)
library(tidyr)
library(ggplot2)
...
k<- unifactorScores(cocor_df_complete[,X1, with = FALSE]) 
h<- unifactorScores(cocor_df_complete[,X2, with = FALSE])
